...
(()=>{const overlay=document.getElementById('aegisBusyOverlay'),label=document.getElementById('aegisBusyText');if(!overlay)return;let pending=0,timer=0;const show=(text='Analyzing system state')=>{pending++;if(label)label.textContent=text;clearTimeout(timer);timer=setTimeout(()=>{if(pending>0){overlay.classList.add('visible');overlay.setAttribute('aria-hidden','false')}},300)};const hide=()=>{pending=Math.max(0,pending-1);if(!pending){clearTimeout(timer);overlay.classList.remove('visible');overlay.setAttribute('aria-hidden','true')}};const reset=()=>{pending=0;clearTimeout(timer);overlay.classList.remove('visible');overlay.setAttribute('aria-hidden','true')};window.AegisBusy={show,hide,reset,run:async(p,text)=>{show(text);try{return await p}finally{hide()}}};document.querySelectorAll('.top-buttons a,.workspace-nav a,.lcars-back').forEach(a=>a.addEventListener('click',()=>{if(a.tagName==='A'){const raw=a.getAttribute('href')||'';try{const target=new URL(a.href,location.href),sameDocument=target.origin===location.origin&&target.pathname===location.pathname&&target.search===location.search;if(sameDocument&&target.hash!==location.hash){reset();return}}catch{}if(raw.startsWith('#')){reset();return}}show('Routing control plane')}));window.addEventListener('hashchange',reset);window.addEventListener('pageshow',reset);})();
